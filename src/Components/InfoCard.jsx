    ...
    <div id="info-card" className="bg-white rounded-3xl p-8 flex flex-col shadow-[0_2px_12px_rgba(74,86,69,0.08)]">
      ...
      <div className="mt-6 space-y-4">
        <div className="flex justify-between">
          <span>Followers</span>
          <span>{userData?.followers ?? "null"}</span>
        </div>

        <div className="flex justify-between">
          <span>Following</span>
          <span>{userData?.following ?? "null"}</span>
        </div>

        <div className="flex justify-between">
          <span>Repositories</span>
          <span>{userData?.public_repos ?? "null"}</span>
        </div>
      </div>

      <div className="h-px bg-[#7A8273] my-6"></div>

      <p className="text-[#6D7566]">{userData?.bio ?? "bio not found"}</p>
    </div>
  );
};
